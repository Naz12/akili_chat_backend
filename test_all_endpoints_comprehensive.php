...
function createAuthRequest($method, $uri, $token, $data = null) {
...
// Chat Sessions
$sessionsRequest = createAuthRequest('GET', "/api/v1/$region/chat/sessions", $token);
...
$sessionsWithMsgRequest = createAuthRequest('GET', "/api/v1/$region/chat/sessions?with_messages=true", $token);
...
if ($testSessionId) {
    // Chat Messages
    $messagesRequest = createAuthRequest('GET', "/api/v1/$region/chat/messages/$testSessionId", $token);
    testEndpoint('GET /chat/messages/{id} (auth)', $sessionsController, 'messages', $messagesRequest, 200, [$testSessionId]);
    
    // Rename Session
    $renameRequest = createAuthRequest('PUT', "/api/v1/$region/chat/sessions/$testSessionId", $token, ['title' => 'Test Rename']);
    testEndpoint('PUT /chat/sessions/{id} (auth)', $sessionsController, 'rename', $renameRequest, 200, [$testSessionId]);
}

// User Profile
$userRequest = createAuthRequest('GET', "/api/v1/$region/user", $token);
...
// Plans
$plansRequest = createAuthRequest('GET', "/api/v1/$region/plans", $token);
...
// Subscription
$subRequest = createAuthRequest('GET', "/api/v1/$region/subscription", $token);
...
// Billing
$billingRequest = createAuthRequest('GET', "/api/v1/$region/billing/current", $token);
...
$upcomingRequest = createAuthRequest('GET', "/api/v1/$region/billing/upcoming-charges", $token);
try {
